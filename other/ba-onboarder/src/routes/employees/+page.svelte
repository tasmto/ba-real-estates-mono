<script lang="ts">
	...
	let busy = false
	let item: RouterInputs['employees']['invite'] | null =
		null // 👈 we're using a helper type
	let errors: { message: string; path: string[] }[] | null =
		null
	...
	const call = async () =>
		await trpc().employees.list.query()
	call()
	...
	const handleSave = async (e: {
		detail: RouterInputs['employees']['invite']
	}) => {
		if (!data.isAuthenticated) {
			needsAuthorization = true
			return
		...
		try {
			const newUser = await trpc().employees.invite.mutate(
				savable(e.detail)
			)
			item = null
			await invalidateAll()
...
<svelte:head>
	<title
		...
		{$t('common.titleSeparator')}
		{$t('common.siteName')}</title
	>
</svelte:head>
