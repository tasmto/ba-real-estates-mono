...
const AreaInput: React.FC<ObjectInputProps<LocationType>> = ({
  ...
  const [areaName, setAreaName] = useState(value?.formatted_address ?? "");
  ...
  return (
    <Stack space={3}>
      ...
      <Text size={1} className="text-gray-400">
        ...
        {value?.formatted_address || 0}
      </Text>

      <Text size={1} className="text-gray-400">
        <span className="text-gray-500">PlaceId:</span> {value?.place_id || 0}
      </Text>
      <Text size={1} className="text-gray-400">
        <span className="text-gray-500">Coordinates:</span> (lat:{" "}
        {value?.coordinates?.lat || "NAN"}) (lng:{" "}
        {value?.coordinates?.lng || "NAN"})
      </Text>

      {loading && (
        <div className="!flex gap-2 rounded-md bg-gray-600 text-gray-200">
          <Spinner muted />
          <span>Loading Address keep typing or please wait...</span>
        </div>
      )}
    </Stack>
  );
